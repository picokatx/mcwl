...
import { Block, BlockLocation, BlockPermutation, BlockProperties, BlockRecordPlayerComponent, BlockType, CommandReturn, Items, ItemStack, MinecraftBlockTypes, Player } from "mojang-minecraft";
        ...
        case 0:
            ...
            for (let i of players) {
                if (i.name == args.get("target")) {
                    let r: PlayerData = PlayerTag.read(i, "dpm:block_interactions");
...
function bellsrungSucceed(suc: string) {
    printStream.success(suc);
}
function bellsrungFail(err: string) {
    printStream.failure(err);
}
function bellsrungInfo(inf: string) {
    printStream.info(inf);
}
const bellsrungCmd = new Command(
    "bellsrung",
