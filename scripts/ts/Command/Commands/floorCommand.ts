...
function floorSucceed(s: string, args: any[]) {
    printStream.success(s, args);
}
function floorFail(s: string, args: any[]) {
    printStream.failure(s, args);
}
function floorInfo(s: string, args: any[]) {
    printStream.info(s, args);
}
const floorCmd = new Command(
    locale.get("cmd_name_floor"),
    locale.get("cmd_description_floor"),
    [
        new CommandFormat([])
    ],
    floor,
