...
function recordSucceed(suc: string) {
    printStream.success(suc);
}
function recordFail(err: string) {
    printStream.failure(err);
}
function recordInfo(inf: string) {
    printStream.info(inf);
}
const recordCmd = new Command(
    "record",
    "Records variety of stuff going on in your world, can replay it",
    [
        new CommandFormat([
